...
async function checkApiStatus() {
  try {
    const res = await fetch("https://stock-dashboard-web-qd9j.onrender.com/api/health", { cache: "no-store" });
...
async function hydrateMarketIndicators() {
  ...
  try {
    const res = await fetch("https://stock-dashboard-web-qd9j.onrender.com/api/market/indicators", { cache: "no-store" });
    if (!res.ok) return;
...
async function hydratePriceHistoryFromApi(stock) {
  ...
  try {
    const res = await fetch(`https://stock-dashboard-web-qd9j.onrender.com/api/stocks/${encodeURIComponent(ticker)}/prices?period=${encodeURIComponent(period)}&t=${Date.now()}`, { cache: "no-store" });
    if (!res.ok) return;
    const payload = await res.json();
...
async function hydrateMetricsFromApi(stock, options = {}) {
  ...
  try {
    ...
    const res = await fetch(`https://stock-dashboard-web-qd9j.onrender.com/api/stocks/${encodeURIComponent(ticker)}/metrics${refreshParam}`, {
      cache: "no-store",
    });
    if (!res.ok) return;
    const payload = await res.json();
...
async function hydrateStockNews(stock, force = false) {
  ...
  if (state.selectedKey === tickerKey(stock)) renderDetail();
  try {
    const res = await fetch(`https://stock-dashboard-web-qd9j.onrender.com/api/stocks/${encodeURIComponent(ticker)}/news-summary?t=${Date.now()}`, { cache: "no-store" });
...
async function renderAiStatus() {
  ...
  try {
    const res = await fetch("https://stock-dashboard-web-qd9j.onrender.com/api/ai/status", { cache: "no-store" });
...
async function renderEconomicCalendar() {
  ...
  try {
    const res = await fetch("https://stock-dashboard-web-qd9j.onrender.com/api/economic-calendar", { cache: "no-store" });
...
async function renderMarketBriefing() {
  ...
  try {
    const res = await fetch("https://stock-dashboard-web-qd9j.onrender.com/api/market/news", { cache: "no-store" });
...
async function renderWatchNewsFromApi(watchedStocks) {
  ...
  if (state.newsScope === "market" || state.newsScope === "macro") {
    requests.push(fetchJsonSafe("https://stock-dashboard-web-qd9j.onrender.com/api/market/news"));
  }
  if ((state.newsScope === "market" || state.newsScope === "watch") && tickers.length) {
    requests.push(fetchJsonSafe(`https://stock-dashboard-web-qd9j.onrender.com/api/watchlist/news?tickers=${encodeURIComponent(tickers.join(","))}`));
  }
...
async function fetchLookupResults(query, queryLower, baseMatches, requestId) {
  ...
  try {
    const res = await fetch(`https://stock-dashboard-web-qd9j.onrender.com/api/search/stocks?q=${encodeURIComponent(query)}`, { cache: "no-store", signal: lookupAbortController.signal });
